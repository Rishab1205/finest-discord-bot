...
async def is_freepack_user(discord_id):
    try:
        ...
        for row in rows:
            if (
                str(row.get("Discord ID")).strip() == str(discord_id)
...
async def send_payment_dm(member, ticket_channel):
    ...
    try:
        embed = discord.Embed(
            title="<a:sc_blue_arrow:1475937927854031073> Payment Confirmed!",
            description=(
...
async def send_join_dm(member):
    try:
        embed = discord.Embed(
            title="<a:sc_blue_arrow:1475937927854031073> Welcome to FINEST DC",
            description=(
